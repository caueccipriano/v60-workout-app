...
function renderProgress(){
  ...
  shell(`<div class="page-head progress-head"><div><span class="page-kicker">xp de força</span><h2>evolução</h2></div><span class="level-chip">nível ${xp.level}</span></div>
    ...
    <section class="xp-grid"><div><span>treinos</span><b>${ss.length}</b></div>${volumeKpi}</section><section class="traco-plus-card"><div class="traco-plus-head"><span>✦ TRAÇO+</span><small>coach inteligente</small></div><h3>${premium.coach}</h3><div class="traco-plus-stats"><div><b>${premium.prs}</b><span>PRs recentes</span></div><div><b>${premium.rising}</b><span>em evolução</span></div><div><b>${premium.workouts}</b><span>treinos analisados</span></div></div><p>análise local do seu histórico · sem alterar seu treino automaticamente</p></section><section class="traco-plus-features"><article><span>◎</span><div><b>Muscle Map</b><small>volume semanal por grupo muscular</small></div><em>ATIVO</em></article><article><span>♛</span><div><b>PR Center</b><small>${premium.prs} recordes nos últimos treinos</small></div><em>ATIVO</em></article><article><span>↗</span><div><b>Relatório inteligente</b><small>${premium.workouts?premium.workouts+' treinos recentes analisados':'aguardando histórico'}</small></div><em>ATIVO</em></article><article><span>⇄</span><div><b>Smart Swap</b><small>trocas equivalentes sem perder progressão</small></div><em>ATIVO</em></article></section>`,{classes:'progress-page'});$('#progressEx').onchange=e=>{state.progressEx=e.target.value;renderProgress()};
}
...
if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=330').catch(()=>{}));
render();