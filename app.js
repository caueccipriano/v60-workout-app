...
function tracoMuscleMapData(){const groups={},ws=weekSessions();ws.forEach(sess=>(sess.exercises||[]).forEach(ex=>{const g=tracoMuscleGroup(ex),sets=(ex.sets||[]).filter(s=>s.done&&!s.skipped).length;groups[g]=(groups[g]||0)+sets;}));return Object.entries(groups).sort((a,b)=>b[1]-a[1]);}
...
function tracoOpenPlusPanel(kind){let title='',body='';if(kind==='muscles'){const rows=tracoMuscleMapData(),max=Math.max(1,...rows.map(r=>r[1]));title='Muscle Map';body=rows.length?'<div class="plus-muscles">'+rows.map(([name,sets])=>'<div><span><b>'+name+'</b><em>'+sets+' séries</em></span><i><u style="width:'+Math.round(sets/max*100)+'%"></u></i></div>').join('')+'</div><p class="plus-note">volume concluído nesta semana · leitura por grupo principal</p>':'<div class="plus-empty">fecha um treino nesta semana para montar seu mapa.</div>';}else if(kind==='prs'){const rows=tracoPRCenterData();title='PR Center';body=rows.length?'<div class="plus-pr-list">'+rows.slice(0,20).map((r,i)=>'<div><span>'+(i+1)+'</span><b>'+tracoEsc(r.name)+'</b><strong>'+r.weight+'kg'+(r.reps?' × '+r.reps:'')+'</strong></div>').join('')+'</div>':'<div class="plus-empty">seu primeiro recorde vai aparecer aqui.</div>';}else{title='Relatório inteligente';const p=tracoPremiumInsights(sessions());body='<div class="plus-report"><strong>'+p.coach+'</strong><p>'+p.workouts+' treinos recentes · '+p.prs+' PRs · '+p.rising+' exercícios em alta</p></div>';}document.body.insertAdjacentHTML('beforeend','<div class="plus-sheet" id="plusSheet"><button class="plus-sheet-backdrop" aria-label="fechar"></button><section><header><div><span>✦ TRAÇO+</span><h2>'+title+'</h2></div><button id="plusSheetClose" aria-label="fechar">×</button></header>'+body+'</section></div>');const close=()=>$('#plusSheet')?.remove();$('#plusSheetClose').onclick=close;$('.plus-sheet-backdrop').onclick=close;}
...
if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=333').catch(()=>{}));
render();