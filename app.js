...
function startSession(workoutId){
  ...
  restoreRestFromSession(state.activeSession);state.page='session';renderSession();return state.activeSession;
}
...
function renderSession(){
  ...
  shell(`<div class="session-topbar"><button class="plain-icon" id="sessionBack">${iconSvg('back')}</button><span>exercício ${state.currentExercise+1} de ${s.exercises.length}</span><button class="plain-icon" id="cancelSession">${iconSvg('close')}</button></div>
    ...
    ${adaptive?`<details class="session-coach"><summary><span>✦ coach</span><b>${adaptive.title}</b><em>+</em></summary><small>${adaptive.text}</small></details>`:''}\n    ${suggestion?`<button class="record-strip" id="useSetSuggestion" type="button"><span><b>${suggestion.label}</b> · usar sugestão</span></button>`:''}
  ...
  if($('#weightInput'))$('#weightInput').oninput=e=>{
    const previous=set.weight;set.weight=e.target.value;
    if(!save(K.draft,s)){set.weight=previous;e.target.value=previous;toast('não consegui salvar a carga');}
  };
  $('#repsInput').oninput=e=>{
...
function restoreRestFromSession(session){
  const endsAt=Number(session?.restEndsAt||0);
  if(endsAt>Date.now()){
    state.restEndsAt=endsAt;state.restRemaining=Math.max(0,Math.ceil((endsAt-Date.now())/1000));
    beginRestTicker();return true;
  }
  state.restEndsAt=0;state.restRemaining=0;
  if(session?.restEndsAt){delete session.restEndsAt;save(K.draft,session);}
  return false;
}
...
function showRestOverlay(){const s=state.activeSession;if(!s){state.restRemaining=0;state.restEndsAt=0;$('#restOverlay')?.remove();return false;}const nextEx=s.exercises?.[state.currentExercise],nextSi=nextEx?currentSetIndex(nextEx):-1,nextLabel=nextEx?`${nextEx.name} · série ${nextSi+1} de ${nextEx.sets.length}`:'próxima série';let el=$('#restOverlay');if(!el){document.body.insertAdjacentHTML('beforeend',`<div class="rest-overlay" id="restOverlay"><div class="rest-inner"><span>DESCANSO</span><strong id="restTime">${fmtClock(state.restRemaining)}</strong><p>próximo: <b>${nextLabel}</b></p><div class="rest-actions"><button id="restMinus" type="button">−15 s</button><button id="restPlus" type="button">+15 s</button></div><button id="skipRest">pular descanso</button></div></div>`);el=$('#restOverlay')}el.classList.add('show');const adjust=delta=>{const previousEndsAt=state.restEndsAt,previousRemaining=state.restRemaining;state.restEndsAt=Math.max(Date.now(),state.restEndsAt+delta*1000);state.restRemaining=Math.max(0,Math.ceil((state.restEndsAt-Date.now())/1000));if(state.activeSession){state.activeSession.restEndsAt=state.restEndsAt;if(!save(K.draft,state.activeSession)){state.restEndsAt=previousEndsAt;state.restRemaining=previousRemaining;state.activeSession.restEndsAt=previousEndsAt;toast('não consegui ajustar o descanso');return;}}const time=$('#restTime');if(time)time.textContent=fmtClock(state.restRemaining);haptic();if(state.restRemaining<=0)stopRest();};const minus=$('#restMinus'),plus=$('#restPlus'),skip=$('#skipRest');if(minus)minus.onclick=()=>adjust(-15);if(plus)plus.onclick=()=>adjust(15);if(skip)skip.onclick=stopRest;return true;}
function beginRestTicker(){clearInterval(state.restTimer);state.restTimer=setInterval(()=>{state.restRemaining=state.restEndsAt?Math.max(0,Math.ceil((state.restEndsAt-Date.now())/1000)):Math.max(0,state.restRemaining-1);const el=$('#restTime');if(el)el.textContent=fmtClock(state.restRemaining);if(state.restRemaining<=0){clearInterval(state.restTimer);if(stopRest()===true){toast('bora pra próxima série');haptic()}}},250)}
function stopRest(){clearInterval(state.restTimer);const s=state.activeSession,previousEndsAt=s?.restEndsAt;if(s?.restEndsAt){delete s.restEndsAt;if(!save(K.draft,s)){s.restEndsAt=previousEndsAt;state.restEndsAt=previousEndsAt;state.restRemaining=Math.max(0,Math.ceil((previousEndsAt-Date.now())/1000));toast('não consegui encerrar o descanso');beginRestTicker();return false;}}state.restRemaining=0;state.restEndsAt=0;$('#restOverlay')?.remove();if(state.page==='session')renderSession();return true}
...
function finishSession(){
  ...
  if(done===0&&!confirm('nenhuma série marcada. encerrar mesmo assim?'))return false;
  const key=tracoDateKey(s.startedAt);
  if(tracoHasDuplicateSession(key,s.workoutId,s.id)&&!confirm('Já existe este treino nessa data. Deseja manter os dois?'))return false;
  ...
  if(!save(K.sessions,next)){toast('não consegui salvar o treino · tente novamente');return false;}
  ...
  $('#restOverlay')?.remove();state.page='finish';renderFinish();return true;
}
...
function tracoMuscleGroup(ex){const id=String(ex?.id||'');if(/supino|crucifixo/.test(id))return 'peito';if(/puxada|remada|pullover/.test(id))return 'costas';if(/elevacao|desenvolvimento|face-pull|crucifixo-inverso/.test(id))return 'ombros';if(/triceps/.test(id))return 'tríceps';if(/rosca/.test(id))return 'bíceps';if(/leg-press|agachamento|extensora/.test(id))return 'quadríceps';if(/rdl|flexora/.test(id))return 'posterior';if(/abdutora/.test(id))return 'glúteos';if(/panturrilha/.test(id))return 'panturrilha';if(/crunch/.test(id))return 'abdômen';return 'outros';}
const TRACO_MUSCLE_TARGETS={peito:[10,16],costas:[12,18],ombros:[10,16],'tríceps':[6,12],'bíceps':[6,12],'quadríceps':[8,14],posterior:[8,14],'glúteos':[4,10],panturrilha:[6,12],'abdômen':[6,12]};
function tracoMuscleMapData(){const groups={},ws=weekSessions();ws.forEach(sess=>(sess.exercises||[]).forEach(ex=>{const g=tracoMuscleGroup(ex),sets=(ex.sets||[]).filter(s=>s.done&&!s.skipped).length;groups[g]=(groups[g]||0)+sets;}));const names=[...new Set([...Object.keys(TRACO_MUSCLE_TARGETS),...Object.keys(groups)])];return names.map(name=>{const sets=groups[name]||0,[min,max]=TRACO_MUSCLE_TARGETS[name]||[0,Infinity];const status=sets<min?'baixo':sets>max?'alto':'alvo';return {name,sets,min,max,status};}).sort((a,b)=>b.sets-a.sets);}
function tracoMuscleCoach(){const rows=tracoMuscleMapData(),low=rows.filter(r=>r.status==='baixo'&&r.min>0),high=rows.filter(r=>r.status==='alto');if(high.length)return 'volume alto em '+high.slice(0,2).map(r=>r.name).join(' e ')+' · priorize recuperação antes de adicionar séries';if(low.length)return low.slice(0,2).map(r=>r.name).join(' e ')+' ainda abaixo da faixa semanal · complete a programação antes de adicionar volume';return 'principais grupos dentro das faixas semanais de referência';}
function tracoPRCenterData(){const best=new Map();sessions().filter(s=>s.finishedAt).forEach(sess=>(sess.exercises||[]).forEach(ex=>(ex.sets||[]).filter(z=>z.done&&!z.skipped&&Number(z.weight)>0).forEach(z=>{const key=tracoExerciseProgressionKey(ex),weight=Number(z.weight),reps=Number(z.reps||0),old=best.get(key);if(!old||weight>old.weight||(weight===old.weight&&reps>old.reps))best.set(key,{name:ex.name,weight,reps,at:sess.startedAt});})));return [...best.values()].sort((a,b)=>b.at-a.at);}
function tracoOpenPlusPanel(kind){let title='',body='';if(kind==='muscles'){const rows=tracoMuscleMapData();title='Muscle Map';body='<div class="plus-muscle-coach"><span>✦ COACH</span><b>'+tracoMuscleCoach()+'</b></div><div class="plus-muscles">'+rows.map(r=>'<div class="'+r.status+'"><span><b>'+r.name+'</b><em>'+r.sets+' séries · alvo '+r.min+'–'+r.max+'</em></span><i><u style="width:'+Math.min(100,Math.round(r.sets/Math.max(1,r.max)*100))+'%"></u></i><small>'+r.status+'</small></div>').join('')+'</div><p class="plus-note">faixas são referências gerais de volume semanal, não prescrição individual. O Traço não adiciona séries sozinho.</p>';}else if(kind==='prs'){const rows=tracoPRCenterData();title='PR Center';body=rows.length?'<div class="plus-pr-list">'+rows.slice(0,20).map((r,i)=>'<div><span>'+(i+1)+'</span><b>'+tracoEsc(r.name)+'</b><strong>'+r.weight+'kg'+(r.reps?' × '+r.reps:'')+'</strong></div>').join('')+'</div>':'<div class="plus-empty">seu primeiro recorde vai aparecer aqui.</div>';}else{title='Relatório inteligente';const p=tracoPremiumInsights(sessions());body='<div class="plus-report"><strong>'+p.coach+'</strong><p>'+p.workouts+' treinos recentes · '+p.prs+' PRs · '+p.rising+' exercícios em alta</p><hr><strong>'+tracoMuscleCoach()+'</strong></div>';}document.body.insertAdjacentHTML('beforeend','<div class="plus-sheet" id="plusSheet"><button class="plus-sheet-backdrop" aria-label="fechar"></button><section><header><div><span>✦ TRAÇO+</span><h2>'+title+'</h2></div><button id="plusSheetClose" aria-label="fechar">×</button></header>'+body+'</section></div>');const close=()=>$('#plusSheet')?.remove();$('#plusSheetClose').onclick=close;$('.plus-sheet-backdrop').onclick=close;}
function renderProgress(){
  ...
  shell(`<div class="page-head progress-head"><div><span class="page-kicker">xp de força</span><h2>evolução</h2></div><span class="level-chip">nível ${xp.level}</span></div>
    ...
    <section class="xp-grid"><div><span>treinos</span><b>${ss.length}</b></div>${volumeKpi}</section><details class="traco-plus-coach"><summary><div><span>✦ TRAÇO+</span><b>coach inteligente</b><small>${premium.coach}</small></div><em>+</em></summary><section class="traco-plus-card"><div class="traco-plus-stats"><div><b>${premium.prs}</b><span>PRs recentes</span></div><div><b>${premium.rising}</b><span>em evolução</span></div><div><b>${premium.workouts}</b><span>treinos analisados</span></div></div><p>análise local do seu histórico · sem alterar seu treino automaticamente</p></section></details><section class="traco-plus-features"><article data-plus="muscles" role="button" tabindex="0"><span>◎</span><div><b>Muscle Map</b><small>volume semanal por grupo muscular</small></div><em>ATIVO</em></article><article data-plus="prs" role="button" tabindex="0"><span>♛</span><div><b>PR Center</b><small>${premium.prs} recordes nos últimos treinos</small></div><em>ATIVO</em></article><article data-plus="report" role="button" tabindex="0"><span>↗</span><div><b>Relatório inteligente</b><small>${premium.workouts?premium.workouts+' treinos recentes analisados':'aguardando histórico'}</small></div><em>ATIVO</em></article><article><span>⇄</span><div><b>Smart Swap</b><small>trocas equivalentes sem perder progressão</small></div><em>ATIVO</em></article></section>`,{classes:'progress-page'});$('#progressEx').onchange=e=>{state.progressEx=e.target.value;renderProgress()};$('[data-plus]').forEach(el=>{el.onclick=()=>tracoOpenPlusPanel(el.dataset.plus);el.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();tracoOpenPlusPanel(el.dataset.plus);}}});
}
...
if('serviceWorker'in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=364').catch(()=>{}));
render();