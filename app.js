const workoutPlan = [
  { id:'seg', day:1, name:'peito + ombros + tríceps · foco inferior', short:'peito + ombro', exercises:[
  ...
  { id:'qui', day:4, name:'peitão + ombros + braços', short:'peito + braços', exercises:[
    ['supino-reto','supino reto','Smith + banco reto',4,6,10,90,'🏋️'],
    ['crucifixo-reto','crossover alto → baixo','Crossover',4,10,15,60,'↘️'],
...
function renderBody(){
  ...
  $('#saveBody').onclick=()=>{const entry={id:Date.now(),date:$('#bodyDate').value,weight:$('#bodyWeight').value,waist:$('#bodyWaist').value,chest:$('#bodyChest').value,arm:$('#bodyArm').value};if(!entry.date)return toast('faltou escolher a data');if(!entry.weight&&!entry.waist&&!entry.chest&&!entry.arm)return toast('faltou preencher pelo menos uma medida');const all=body();all.push(entry);save(K.body,all);toast('medição salva');haptic();renderBody()};
