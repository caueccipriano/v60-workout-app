...
function v60GuidePreview(ex){
  ...
  return `<button class="exercise-guide-preview v60-v4-preview" id="openExerciseGuide" type="button" aria-label="ver execução de ${tracoMediaEsc(ex.name)}"><div class="v60-v4-motion-wrap">${v60V4MotionSvg(ex,false)}<span class="v60-v4-loop-badge"><i></i> movimento exato</span></div><span class="exercise-guide-caption"><span><b>ver execução</b><small>${tracoMediaEsc(g.cue)}</small></span><i>↗</i></span></button>`;
}
function v60ShowGuide(ex){
  v60CloseGuide();
  const g=v60GuideFor(ex),scene=v60V4Scene(ex),setCount=tracoMediaSetCount(ex),spec=TRACO_MEDIA_AUDIT[ex.id];
  document.body.classList.add('v60-guide-open');
  document.body.insertAdjacentHTML('beforeend',`<div class="v60-guide-backdrop" id="v60GuideBackdrop"></div><aside class="v60-guide-sheet v60-v4-sheet" id="v60GuideSheet" role="dialog" aria-modal="true" aria-label="execução de ${tracoMediaEsc(ex.name)}"><div class="v60-guide-handle"></div><button class="v60-guide-close" id="v60GuideClose" aria-label="fechar">×</button><div class="v60-guide-content"><span class="v60-guide-kicker">execução técnica validada</span><h2>${tracoMediaEsc(ex.name)}</h2><p class="v60-guide-equipment">${tracoMediaEsc(ex.equipment)}</p><div class="v60-v4-motion-wrap v60-v4-motion-large">${v60V4MotionSvg(ex,true)}<span class="v60-v4-loop-badge"><i></i> início ↔ fim · exato</span></div><p class="v60-v4-tech-note">${tracoMediaEsc(scene.note)}</p><div class="v60-guide-tags"><span>${tracoMediaEsc(g.focus)}</span><span>${setCount} × ${tracoMediaEsc(ex.min)}-${tracoMediaEsc(ex.max)}</span><span>${tracoMediaEsc(ex.rest)}s descanso</span></div><section class="v60-guide-tips"><h3>dicas rápidas</h3><ol>${g.tips.map(t=>`<li>${tracoMediaEsc(t)}</li>`).join('')}</ol></section><section class="v60-guide-error"><span>erro comum</span><p>${tracoMediaEsc(g.error)}</p></section><p class="v60-video-license-note">Traço Exact Media · exercício, equipamento e trajetória auditados${spec?.scene?` · ${tracoMediaEsc(spec.scene)}`:''}.</p><button class="cta-lime" id="v60GuideDone">voltar pro treino</button></div></aside>`);
